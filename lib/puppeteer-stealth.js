...
const ADBLOCKER_CACHE_PATH = appRootPath + '/storage/adblockcache.bin'
...
class StealthPage {
  ...
  async onLoad(page) {
    await this.enableAdblockToPage(page)

    return page
  }
  async onCreate(page) {
    await this.emulateToPage(page)
    await this.emulateMediaTypeToPage(page)
    await this.emulateTimezoneToPage(page)
    await this.emulateResourceBlockingToPage(page)
    await this.injectChromeWindowObjectToPage(page)
    await this.consoleDebugToPage(page)
    await this.injectIframeContentWindowToPage(page)
    await this.injectMediaCodecToPage(page)
    await this.injectNavigatorLanguageToPage(page)
    await this.injectNavigatorPermissionToPage(page)
    await this.injectNavigatorPluginToPage(page)
    await this.injectNavigatorVendorToPage(page)
    await this.injectNavigatorWebdriverToPage(page)
    await this.overrideUseragentToPage(page)
    await this.injectWebglVendorToPage(page)
    await this.injectWindowOuterDimesionToPage(page)

    return page
  }

  async enableAdblockToPage(page) {
    let blocker

    try {
      blocker = await PuppeteerBlocker.deserialize(
        new Uint8Array(await fs.readFile(ADBLOCKER_CACHE_PATH))
      )
    } catch (err) {
      ...
      await fs.writeFile(ADBLOCKER_CACHE_PATH, blocker.serialize())
    }
  ...
  async consoleDebugToPage(page) {
    await page.evaluateOnNewDocument(() => {
      console.debug('n')
    })
  }

  async injectChromeWindowObjectToPage(page) {
    await page.evaluateOnNewDocument(
      args => {
  ...
  async injectIframeContentWindowToPage(page) {
    await page.evaluateOnNewDocument(() => {
      try {
  ...
  async injectMediaCodecToPage(page) {
    await page.evaluateOnNewDocument(() => {
      try {
        /**
  ...
  async injectNavigatorLanguageToPage(page) {
    await page.evaluateOnNewDocument(() => {
  ...
  async injectNavigatorPermissionToPage(page) {
    await page.evaluateOnNewDocument(() => {
  ...
  async injectNavigatorPluginToPage(page) {
    await page.evaluateOnNewDocument(() => {
  ...
  async injectNavigatorVendorToPage(page) {
  ...
  async injectNavigatorWebdriverToPage(page) {
    await page.evaluateOnNewDocument(() => {
  ...
  async overrideUseragentToPage(page) {
    page._client.send('Network.setUserAgentOverride', {
      userAgent: this.opts.user_agent.replace(
        'HeadlessChrome/',
        'Chrome/'
      ),
      acceptLanguage: 'en-US,en,id',
      platform: 'Win32'
    })
  }

  async injectWebglVendorToPage(page) {
    await page.evaluateOnNewDocument(() => {
      try {
  ...
  async injectWindowOuterDimesionToPage(page) {
    await page.evaluateOnNewDocument(() => {
      try {
  ...
  async emulateMediaTypeToPage(page) {
    await page.emulateMediaType('screen')
  }

  async emulateTimezoneToPage(page) {
    await page.emulateTimezone('Asia/Jakarta')
  }

  async emulateResourceBlockingToPage(page) {
    try {
      await page.setRequestInterception(true)
    } catch (err) {
      console.log('Error while intercepting request', err)
    }
