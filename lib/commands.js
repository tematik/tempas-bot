...
async function commandBuyShopee({ browser, credential, url }) {
  ...
  try {
    await notifier.log('buy:shopee:log', { message: 'Checking login status' })
  ...
  try {
    await notifier.log('buy:shopee:log', { message: 'Redirecting to product page' })
  ...
  const runProcess = async () => {
    await notifier.log('buy:shopee:log', { message: 'Finding for add to cart button' })
    ...
    let result = await page.evaluate(() => {
      ...
      if (icon && icon.parentElement && icon.parentElement.nodeName && icon.parentElement.nodeName.toUpperCase() === 'BUTTON') {
        ...
        icon.parentElement.classList.forEach(v => {
          if (v.indexOf('disabled') >= 0) {
            isDisabled = true
          }
        })

        if (isDisabled) {
          return 'Add to cart button is disabled. Reloading ...'
        }
      ...
      return 'Add to cart button does not exists. Reloading ...'
    })

    if (typeof result === 'string') {
      console.log(result)
      await delay(result === 'Add to cart button is disabled. Reloading ...' ? 1500 : 1000)
      await page.reload({
        timeout: 60000,
        waitUntil: 'domcontentloaded'
      })
